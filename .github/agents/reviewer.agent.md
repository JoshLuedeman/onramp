...
- **Coverage Threshold:** 75% line coverage both sides; 80%+ for services/routes; 90%+ for critical paths; frontend also 70% branch, 60% function coverage
- **PR Merge Criteria:** All CI checks must pass (lint, test, build, coverage thresholds, Bicep validation); no `any` types in TypeScript; no `print()` in Python; tests cover all new/modified code; no hardcoded secrets
