...
## Project Knowledge
- **Tech Stack:** React 19 + TypeScript 5.9 + Vite 7 + Fluent UI React v9 (frontend), Python 3.12 + FastAPI + SQLAlchemy 2.0 async + Alembic (backend), Azure SQL / SQLite (database), Azure AI Foundry (LLM), Bicep (IaC), Azure Container Apps (hosting), GitHub Actions (CI/CD)
- **Languages:** TypeScript, Python, Bicep
- **ADR Location:** `docs/architecture.md` for architecture decisions; `.teamwork/docs/decisions/` for ADRs
- **Key Architectural Constraints:** Must remain fully functional in dev mode without Azure credentials (mock auth, SQLite, mock AI); all frontend API calls through `/api/*` relative URLs (Vite proxies to backend); no `any` types in TypeScript; no `print()` in Python; all I/O must be async; Fluent UI v9 only (never v8); CORS origins must be JSON array format
- **Existing Patterns to Follow:** Services as singletons at module level; Pydantic v2 models for all API validation; lazy-loaded pages with `React.lazy()` + `<Suspense>`; `makeStyles` + `tokens` for styling; all API calls through `frontend/src/services/api.ts`; routes in `backend/app/api/routes/` registered in `main.py`; dev mode detection via empty `ONRAMP_AZURE_TENANT_ID`
