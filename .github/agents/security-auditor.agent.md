...
- **Dependency Audit Command:** `cd frontend && npm audit` (frontend); `cd backend && pip-audit` (backend)
- **Secrets Patterns to Watch:** `ONRAMP_` prefixed env vars (especially `ONRAMP_AZURE_CLIENT_SECRET`, `ONRAMP_AI_FOUNDRY_KEY`, `ONRAMP_DATABASE_URL`); Azure AD client secrets; AI Foundry API keys; DB connection strings; any values in `.env` files
- **Auth Mechanism:** Microsoft Entra ID / MSAL (msal-browser, msal-react for frontend; msal-python for backend); token validation in `backend/app/auth/`; RBAC via `RoleChecker` dependency (admin, architect, viewer roles)
- **Known Sensitive Data Patterns:** Azure credentials and AI API keys in env vars; user data in `users` table; security headers middleware enforces HSTS, X-Frame-Options DENY, X-Content-Type-Options nosniff, CSP; CORS explicitly configured (never `allow_origins=["*"]` in production); all secrets stored in Azure Key Vault in production
