...
## Project Knowledge
- **API Framework:** FastAPI (Python 3.12), async with uvicorn
- **API Style:** REST; routes in `backend/app/api/routes/` (9 modules), registered in `backend/app/main.py`; Pydantic v2 schemas in `backend/app/schemas/` for all request/response validation
- **OpenAPI Spec Location:** Auto-generated at `http://localhost:8000/docs` (Swagger UI) and `http://localhost:8000/openapi.json` when backend is running; see also `docs/api.md`
- **Dev Server Command:** `./dev.sh up` (Docker Compose: backend on :8000, frontend on :5173); or `cd backend && uvicorn app.main:app --reload --port 8000`
- **API Test Command:** `cd backend && pytest tests/ -v --cov=app --cov-fail-under=75`
