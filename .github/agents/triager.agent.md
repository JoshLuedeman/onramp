...
## Project Knowledge
- **Issue Tracker:** GitHub Issues — https://github.com/JoshLuedeman/onramp/issues
- **Label Taxonomy:** `bug`, `enhancement`, `documentation`, `frontend`, `backend`, `infra`, `security`, `dependencies`, `good first issue`, `help wanted`
- **Priority Levels:** critical (blocking deployment/core workflow), high (major feature/significant bug), medium (standard feature/non-blocking bug), low (nice-to-have/cosmetic)
- **Workflows:** bug → `fix/` branch → PR with tests; feature → `feat/` branch → PR with tests; docs → `docs/` branch → PR; refactor → `refactor/` branch → PR with tests; all PRs require CI green + coverage thresholds met
