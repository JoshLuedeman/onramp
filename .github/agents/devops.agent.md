...
## Project Knowledge
- **CI/CD Platform:** GitHub Actions (Node 24, Python 3.12); workflows in `.github/workflows/` (`ci.yml` for build/lint/test, `deploy.yml` for Azure deployment)
- **Cloud Provider:** Azure (Azure Container Apps, Azure SQL, Azure AI Foundry, Azure Key Vault)
- **IaC Tool:** Bicep (Azure-native); modules in `infra/modules/`, composed by `infra/main.bicep`; validate with `az bicep build --file infra/main.bicep`
- **Container Runtime:** Docker; `docker-compose.yml` for local dev (`./dev.sh up`); `frontend/Dockerfile` and `backend/Dockerfile` for container builds
- **Orchestration:** Azure Container Apps (production); Docker Compose (development)
- **Build Command:** `cd frontend && npm run build` (frontend); backend runs via uvicorn (no build step)
- **Deploy Command:** Azure Container Apps deployment via GitHub Actions `deploy.yml`; ARM template at `infra/azuredeploy.json` for Deploy to Azure button
