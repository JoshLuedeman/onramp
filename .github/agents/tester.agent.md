...
## Project Knowledge
- **Test Framework:** pytest + pytest-asyncio + httpx AsyncClient (backend, 35 files, 245+ tests); Vitest + @testing-library/react + @testing-library/user-event (frontend, 24 files, 122+ tests)
- **Test Command:** `cd backend && pytest tests/ -v --cov=app --cov-fail-under=75` (backend); `cd frontend && npm run test:coverage` (frontend)
- **Coverage Tool:** pytest-cov (backend), @vitest/coverage-v8 (frontend)
- **Coverage Threshold:** 75% line coverage both sides; 80%+ for services/routes; 90%+ for critical paths (questionnaire flow, architecture generation, compliance scoring, deployment orchestration); frontend also requires 70% branch, 60% function coverage
- **Test Directory Layout:** `backend/tests/test_*.py` (flat directory); frontend tests colocated as `*.test.{ts,tsx}` next to source files
- **CI Test Command:** `pytest tests/ -v --cov=app --cov-fail-under=75` (backend); `npm run test:coverage` (frontend); both enforced in GitHub Actions CI
