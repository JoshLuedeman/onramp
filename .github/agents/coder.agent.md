...
## Project Knowledge
- **Tech Stack:** React 19 + TypeScript 5.9 + Vite 7 + Fluent UI React v9 (frontend), Python 3.12 + FastAPI + SQLAlchemy 2.0 async + Alembic (backend), Azure SQL / SQLite (database), Bicep (IaC), Azure Container Apps (hosting)
- **Languages:** TypeScript, Python, Bicep
- **Package Manager:** npm (`frontend/package.json`), pip with pyproject.toml (`backend/pyproject.toml`)
- **Build Command:** `cd frontend && npm run build` (frontend); backend has no build step (runs via uvicorn)
- **Test Command:** `cd backend && pytest tests/ -v --cov=app --cov-fail-under=75` (backend); `cd frontend && npm run test:coverage` (frontend)
- **Lint Command:** `cd backend && ruff check app/` (backend); `cd frontend && npm run lint` (frontend)
- **PR Conventions:** Conventional Commits required (`feat:`, `fix:`, `docs:`, `refactor:`, `test:`, `chore:`); link issue with "Closes #N"; all CI checks must pass (lint, test, build, coverage thresholds); one task per PR
