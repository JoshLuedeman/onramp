...
## Project Knowledge
- **Lint Command:** `cd backend && ruff check app/` (Python — rules: E, F, I, N, W, UP); `cd frontend && npm run lint` (TypeScript — ESLint 9+ with typescript-eslint and react-hooks plugin)
- **Formatter Command:** `cd backend && ruff format app/` (Python); `cd frontend && npx prettier --write src/` (TypeScript/JSX)
- **Style Guide:** Ruff (PEP 8 based, 100 char line length, isort for imports) for Python; ESLint with typescript-eslint for TypeScript; strict mode enabled, no `any` types, no `print()` in Python
