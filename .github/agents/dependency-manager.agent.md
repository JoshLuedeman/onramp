...
- **Dependency Manifest:** `frontend/package.json` (frontend), `backend/pyproject.toml` (backend)
- **Lockfile:** `frontend/package-lock.json` (frontend); backend uses pip (no lockfile, deps in pyproject.toml)
- **Audit Command:** `cd frontend && npm audit` (frontend); `cd backend && pip-audit` (backend)
